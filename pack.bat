@ECHO OFF
	...
	FOR /D %%G IN (.\*) DO (
		...
		IF EXIST "manifest.xml" (
			pakman --compile --source=.\ --optimize --output=..\bin\org.santedb.%%~nxG.pak --keyFile=..\..\keys\community.santesuite.net.pfx --keyPassword=..\..\keys\community.santesuite.net.pass --embedcert --install
		)
		POPD
	)
