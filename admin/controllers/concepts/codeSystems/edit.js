...
angular.module('santedb').controller('EditCodeSystemController', ["$scope", "$rootScope", "$state", "$stateParams", "$timeout", function ($scope, $rootScope, $state, $stateParams, $timeout) {
    ...
    $scope.downloadCodeSystem = function(id) {
        window.open(`/hdsi/CodeSystem/${id}/_export?_include=ReferenceTerm:codeSystem%3d${id}&_include=Concept:referenceTerm.term.codeSystem%3d${id}%26_exclude=relationship%26_exclude=conceptSet%26_exclude=referenceTerm&_exclude=versionSequence&_exclude=replacesVersion&_include=ConceptReferenceTerm:term.codeSystem%3d${id}`, '_blank').onload = function (e) {
            win.close();
        };
    }
}]);
