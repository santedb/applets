...
angular.module('santedb').controller('EditConceptSetController', ["$scope", "$rootScope", "$state", "$stateParams", "$timeout", function ($scope, $rootScope, $state, $stateParams, $timeout) {
    ...
    $scope.downloadConceptSet = function (id) {
        var win = window.open(`/hdsi/ConceptSet/${id}/_export?_include=Concept:conceptSet%3d${id}%26_exclude=conceptSet%26_exclude=referenceTerm&_include=ConceptReferenceTerm:source.conceptSet%3d${id}&_include=ReferenceTerm:concept.source.conceptSet%3d${id}&_includesFirst=false&_include=ConceptSet:compose.source=${id}%26_include=concept`, '_blank');
