...
angular.module('santedb').controller('MatchConfigurationDetailController', ["$scope", "$rootScope", "$state", "$stateParams", "$timeout", "$interval", function ($scope, $rootScope, $state, $stateParams, $timeout, $interval) {
    ...
    $scope.enable = async function () {
        if (confirm(SanteDB.locale.getString("ui.admin.matches.config.enable.confirm", { id: $scope.matchConfiguration.id }))) {
            try {
                SanteDB.display.buttonWait(`#btnEnable`, true);
                await updateConfiguration($scope.matchConfiguration.id, true);
                toastr.success(SanteDB.locale.getString("ui.admin.matches.config.enable.success", { id: $scope.matchConfiguration.id }));
            }
            catch (e) {
                toastr.error(SanteDB.locale.getString("ui.admin.matches.config.enable.error", { id: $scope.matchConfiguration.id, e: e.message }));
            }
            finally {
    ...
    $scope.disable = async function () {
        if (confirm(SanteDB.locale.getString("ui.admin.matches.config.disable.confirm", { id: $scope.matchConfiguration.id }))) {
            try {
                SanteDB.display.buttonWait(`#btnDisable`, true);
                await updateConfiguration($scope.matchConfiguration.id, false);
                toastr.success(SanteDB.locale.getString("ui.admin.matches.config.disable.success", { id: $scope.matchConfiguration.id }));
            }
            catch (e) {
                toastr.error(SanteDB.locale.getString("ui.admin.matches.config.disable.error", { id: $scope.matchConfiguration.id, e: e.message }));
            }
            finally {
    ...
    $scope.save = async function (form) {
        ...
        try {
            ...
            if (!$scope.scopedObject.uuid) {
                ...
                $state.transitionTo("santedb-admin.admin.config.view", { id: $scope.scopedObject.id });
            }
            else {
            ...
            toastr.success(SanteDB.locale.getString("ui.admin.matches.config.save.success", { id: $scope.scopedObject.id }));

        }
        catch (e) {
            toastr.error(SanteDB.locale.getString("ui.admin.matches.config.save.error", { id: $scope.scopedObject.id }));
            $rootScope.errorHandler(e);
        }
    }
    ...
    $scope.runTest = async function (testForm) {
        ...
        try {
            ...
            $("#runTest").html(`<i class='fas fa-circle-notch fa-spin'></i> ${SanteDB.locale.getString("ui.admin.matches.config.test.runningMatch")}`);
            ...
            $("#runTest").html(`<i class='fas fa-circle-notch fa-spin'></i> ${SanteDB.locale.getString("ui.admin.matches.config.test.gatheringResults")}`);
    ...
    $scope.navToMatchDetail = function(parms) {
        var viewer = SanteDB.application.getResourceViewer("Match");
        if(viewer){
            viewer($state, parms);
        }
        else {
            alert(SanteDB.locale.getString("ui.admin.matches.config.test.nav.missing"));
        }
    }
    ...
    $scope.matchDetail = async function (id) {
        try {
            ...
            var matchReport = await SanteDB.resources.patient.getAssociatedAsync(id, "match-candidate", $scope.scopedObject._test.input, { _configuration: $scope.scopedObject.id, _upstream: true });
